...
final class DefaultValidateNicknameUseCase: ValidateNicknameUseCase {
    private static let specialCharRegex: NSRegularExpression = {
        (try? NSRegularExpression(pattern: "[^\\p{L}\\p{N}]", options: [])) ?? NSRegularExpression()
    }()
    ...
    func execute(nickname: String, currentName: String?) -> NicknameError {
        ...
        if nickname.isEmpty {
            ...
            return .length
        } else if nickname.unicodeScalars.contains(where: { $0.properties.isWhitespace }) {
            return .containsBlank
        } else if let currentName, currentName == nickname {
            return .duplicate
        } else {
            let range = NSRange(location: 0, length: nickname.utf16.count)
            if Self.specialCharRegex.firstMatch(in: nickname, options: [], range: range) != nil {
